* EMACS ER SETUP
Emacs er nijer language [[https://en.wikipedia.org/wiki/Emacs_Lisp][emacs lisp]] diye lekha. To emacs er main config file ta edit korar maddhome puro emacs ta k e customize kora jai. Emacs customize korar jonno mainly 2 ta file customize kora lage. init.el abong early-init.el.

Ami emacs configure korar jonno org mode file use kori. Ay jinis ta ami [[https://www.youtube.com/watch?v=d1fgypEiQkE&list=PL5--8gKSku15e8lXf7aLICFmAHQVo0KXX][disto tube]] er customization er theke sikhci abong tarpor theke e ami amr full emacs er setup org file a kori.

* EMACS KIVABE KAJ KORE
emacs mainly to emacs-lisp er upor kaj kore, emacs mainly package er upor kaj kore, jemon arch ba debian er package repo er theke package download kora jai thik temon er emacs er repo er theke j j package dorkar oi gula download kore emacs customize korte hoi. Ami amr config a total 4 ta repo use kori [[https://github.com/progfolio/elpaca][elpaca]], [[https://melpa.org][melpa]], [[https://elpa.gnu.org/][elpa]] abong [[https://orgmode.org/][org]]. Ami sob package gulo ay repo theke e download kori. 
...
* CONFIG.ORG 
Akhn hocce emacs er main configuretion suru akhn theke amra emacs config kora suru korbo. To first a amader package install korar jonno repo gulo add korte hobe. To aytar jonno amra repo gula add korbo:-
** ALL THE REPOS
Ay block tar vitore sob repo gula add korbo first a elpaca. elpaca er repo te sob kicu details a bola ace j ki kora lagbe. elpaca ay vabe setup korte hobe, just repo theke code ta k niye ase paste kore dile e enough.
*** ELPACA
#+begin_src emacs-lisp
...
*** OTHER REPO'S
Baki j repo gula ace oi gula k alada kore add kore add kora jai.
#+begin_src emacs-lisp
(require 'package)
...
Emacs er defult white theme ta onk besi cokhe lage oitar jonno doom er theme abong doom modeline, details gula paoyar jonno theme install kora.
...
** FONTS 
Emacs a font add korar jonno first a [[https://github.com/NOTHING-R/INSTALL_SCRIPT][install script]] theke fonts gula install korte hobe na hoile error asbe j JetBrains Mono-11 font ta error dekhabe. 
...
** ORG MODE
Emacs bebohar r akta main jinis hocce org mode. Org mode bebohar er jonno
...
;;   "Copy org-mode link at point as [[link][description]] to kill-ring."
;;   (interactive)
...
;;                (desc-text (if desc (car desc) url)))
;;           (kill-new (format "[[%s][%s]]" url desc-text))
;;           (message "Copied: [[%s][%s]]" url desc-text))
